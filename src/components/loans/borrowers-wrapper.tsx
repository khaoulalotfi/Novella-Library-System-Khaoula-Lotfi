...
export function BorrowersWrapper({ loans, subscribers, books }: IProps) {
  ...
  return (
    <div className="p-6">
      <div className="rounded-xl bg-gradient-to-r from-primary/20 to-primary/5 border border-primary/20 p-6 mb-8">
        <h1 className="text-3xl font-bold text-primary">Book Borrowers</h1>
        <p className="text-muted-foreground mt-1">
          Subscribers currently borrowing books
        </p>
        <div className="flex items-center gap-x-2 mt-3">
          <Badge variant="outline" className="text-primary border-primary/40">
            {`${filtered.length} ${filtered.length === 1 ? "Borrower" : "Borrowers"} Active`}
          </Badge>
        </div>
      </div>
      <div className="mb-4">
